...
export default function PostCard({ post, onVoteChange }: PostCardProps) {
  ...
  const [currentImage, setCurrentImage] = useState(post.images && post.images.length > 0 ? post.images[0].image_url : null);
                ...
                {post.images.map((image) => (
                  <img
                    ...
                    className={`h-16 w-16 object-cover rounded cursor-pointer ${
                      currentImage === image.image_url ? 'ring-2 ring-primary' : ''
                    }`}
        ...
        <div className="flex space-x-4">
          <HoverCard>
            <HoverCardTrigger asChild>
              <Button 
                variant={post.user_vote?.vote_type === 'upvote' ? 'default' : 'outline'} 
                size="sm"
          ...
          <HoverCard>
            <HoverCardTrigger asChild>
              <Button 
                variant={post.user_vote?.vote_type === 'downvote' ? 'default' : 'outline'} 
                size="sm"
